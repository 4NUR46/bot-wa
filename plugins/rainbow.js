...
let handler  = async (m, { conn, text }) => {
    ...
    if (/image/.test((m.quoted ? m.quoted : m.msg).mimetype || '')) {
      ...
      let p = await (await fetch('https://some-random-api.ml/canvas/gay?avatar='+encodeURIComponent(tek))).buffer()
  conn.sendFile(m.chat, p, 'gta.png' , 'anjay!!!', m, false, { thumbnail: Buffer.alloc(0) })
}}
handler.help = ['rainbow <reply img>']
handler.tags = ['maker']
