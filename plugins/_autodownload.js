...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        tiktok(m.text).then(async res => {
            ...
            await this.sendVideo(m.chat, json.nowm, '*KYAAA*', m, { thumbnail: buf })
        }).catch(_ => _)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await this.sendVideo(m.chat, json.download, `*KYAAA*`, m, { thumbnail: buf })
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text).then(async res => {
            ...
            await this.sendVideo(m.chat, json.data[1].url, '*kyaaa*', m, buf)
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(m.text).then(async res => {
            ...
            await this.sendVideo(m.chat, json.data.url, `*KYAAA*`, m, { thumbnail: buf })
        }).catch(_ => _)
    }
