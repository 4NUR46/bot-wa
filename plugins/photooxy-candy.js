...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
	...
	let text = args.join` `
	fetch('https://tessyyy-api.herokuapp.com/api/textmaker/metallic?text=' + encodeURIComponent(text) + '&theme=candy' )
    	.then(res => res.json())
    	.then(json => {
...
handler.help = ['candy'].map(v => v + '[text]')
handler.tags = ['ph']
handler.command = /^(candy)$/i
handler.owner = false
handler.mods = false
